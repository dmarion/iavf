...
static void i40evf_get_strings(struct net_device *netdev, u32 sset, u8 *data)
{
	...
	if (sset == ETH_SS_STATS) {
		...
		for (i = 0; i < (int)I40EVF_PRIV_FLAGS_STR_LEN; i++) {
				 ...
				 struct ethtool_rxfh_indir *indir)
{
	...
	if (indir->size < adapter->rss_lut_size)
		return -EINVAL;
	 ...
	 */
	for (i = 0; i < adapter->rss_lut_size; i++)
		indir->ring_index[i] = (u32)adapter->rss_lut[i];
	indir->size = adapter->rss_lut_size;
				 ...
				 const struct ethtool_rxfh_indir *indir)
{
	 ...
	 */
	for (i = 0; i < adapter->rss_lut_size; i++)
		adapter->rss_lut[i] = (u8)(indir->ring_index[i]);
